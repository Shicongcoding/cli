...
wps365-cli 支持两种认证模式，命令根据底层 OpenAPI `security` 声明自动选择，`--token-type` 可显式覆盖。模式不兼容时直接报错，不静默切换。

| 模式 | OAuth Grant | 适用场景 | 获取方式 |
|------|------------|---------|---------|
| `delegated` | Authorization Code (browser) | 当前用户信息、个人日历、个人邮件等用户态接口 | `auth login --scopes "kso.user_base.read,kso.calendar.read"` |
...
3. 授权完成后浏览器重定向到 callback URL，携带 `code`
4. CLI 用 `code` + `client_secret` 换取 `access_token` 和 `refresh_token`
...
| 事件 | Delegated | App |
|------|-----------|-----|
| 过期前 10 秒 | 用 `refresh_token` 刷新 | 用 `client_credentials` 重新获取 |
| 401 响应 | 透明刷新 + 重试 | 透明重新获取 + 重试 |
| Refresh token 过期 | 提示重新 `auth login` | 不适用（无 refresh token） |
...
不匹配时 CLI 直接报错，不静默切换模式。这确保了用户明确知道当前操作使用的身份。

## 安全考量

- OAuth callback 仅监听 `localhost`，不暴露到公网
